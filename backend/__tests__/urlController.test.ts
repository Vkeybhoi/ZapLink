...
describe("UrlService", () => {
  ...
  test("decode should throw an error for a non-existent short URL", async () => {
    ...
    await expect(urlService.decode(nonExistentShortUrl)).rejects.toThrow(
      "Short URL not found"
    );
  });
  ...
  test("getStats should throw an error for a non-existent short URL", async () => {
    ...
    await expect(urlService.getStats(nonExistentShortPath)).rejects.toThrow(
      "Short URL not found"
    );
  });
  ...
  test("redirect should throw an error for a non-existent short URL", async () => {
    ...
    await expect(urlService.redirect(nonExistentShortPath)).rejects.toThrow(
      "Short URL not found"
    );
  });
});